...
def generate_today_orders(project_dir: str = ".", n_orders: int = 100) -> None:
            ...
            new_inv_rows.append(
                {
                    ...
                    "product_distribution_center_id": prod.get(
                        "distribution_center_id"
                    ),
                }
            )
    ...
    print(
        f"[SUCCESS] {today.date()}: appended {len(new_items_df)} items across {n_orders} orders"
    )
    print(
        f"  raw_order_items    : {len(existing_items):,} → {len(updated_items):,} rows"
    )
    print(
        f"  raw_orders         : {len(existing_orders):,} → {len(updated_orders):,} rows"
    )
